...
def transfer_data(source: str, dest: str, num_row: int):
    ...
    if num_row <= len(lines):
        with open(dest, "a", encoding="utf-8") as dest_file:
            dest_file.write(lines[num_row - 1])

        print(f"Строка номер {num_row} успешно скопирована из {source} в {dest}")
    else:
...
while True:
    ...
    if mode == 1:
        ...
        destination_file = input("Введите имя файла, в который нужно вставить данные: ")
        row_number = int(input("Введите номер строки для копирования: "))
        transfer_data(source_file, destination_file, row_number)
